...
#[cfg(target_os = "linux")]
pub fn create_rgba8_headless(headless_ctx: &backend_window::HeadlessContext,
                             width: u32,
                             height: u32)
    -> DeviceInitParams
{   
    ...
    let (device, factory) = backend::create(|s| headless_ctx.get_proc_address(s) as *const c_void);
