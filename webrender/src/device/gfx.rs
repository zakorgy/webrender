...
impl<B: hal::Backend> CommandPool<B> {
    ...
    fn acquire_command_buffer(&mut self) -> &mut hal::command::CommandBuffer<B, hal::Graphics> {
        if self.next_free >= self.command_buffers.len() {
            let command_buffer = self.command_pool.acquire_command_buffer::<hal::command::OneShot>();
            self.command_buffers.push(command_buffer);
        }
        self.next_free += 1;
        &mut self.command_buffers[self.next_free - 1]
    }

    fn command_buffers(&self) -> &[hal::command::CommandBuffer<B, hal::Graphics>] {
        &self.command_buffers[0 .. self.next_free]
    }
...
impl<B: hal::Backend> Device<B> {
    ...
    pub fn bind_draw_target(
        ...
        {
            let cmd_buffer =
                self.command_pool[self.next_id].acquire_command_buffer();
            let rbos = &self.rbos;
