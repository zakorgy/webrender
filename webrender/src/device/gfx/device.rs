...
pub enum DrawTargetUsage {
    // Only a clear operation is apllied to the target
    ClearOnly,
    // Only a blit or copy operation is applied to the target
    CopyOnly,
    // The target will be drawn but clear and copy is also possible
    Draw,
}
...
impl<B: hal::Backend> Device<B> {
    pub fn new(
        ...
        let mut command_pool: SmallVec<[CommandPool<B>; 1]> = SmallVec::new();
    ...
    fn draw(&mut self) {
        self.bind_textures();
        self.bind_uniforms();
        let (frame_buffer, format, (depth_img, depth_test_changed)) = if self.bound_draw_fbo != DEFAULT_DRAW_FBO {
    ...
    pub fn bind_cache_buffer(&mut self, texture: &Texture) {
        ...
        if let Some(barrier) = buffer.transit(hal::buffer::Access::SHADER_READ, true) {
            unsafe {
                cmd_buffer.pipeline_barrier(
                    PipelineStage::HOST .. PipelineStage::VERTEX_SHADER | PipelineStage::FRAGMENT_SHADER,
        ...
        if let Some(barrier) = buffer.transit(hal::buffer::Access::HOST_WRITE | hal::buffer::Access::HOST_READ, false) {
            unsafe {
                cmd_buffer.pipeline_barrier(
                    PipelineStage::VERTEX_SHADER | PipelineStage::FRAGMENT_SHADER .. PipelineStage::HOST,
                    hal::memory::Dependencies::empty(),
                    &[barrier],
                );
            }
        ...
        if self.bound_draw_fbo != fbo_id {
            ...
            if transit_back_old_image {
                ...
                let pipeline_stages = match old_usage {
                    DrawTargetUsage::ClearOnly => PipelineStage::COLOR_ATTACHMENT_OUTPUT
    ...
    fn is_draw_target(&self, id: TextureId) -> bool {
        if let Some(fbo) = self.fbos.get(&self.bound_draw_fbo) {
            return fbo.texture_id == id
        }
        false
    }
    ...
    fn clear_target_rect(
        ...
        let (frame_buffer, format, depth_img) = if self.bound_draw_fbo != DEFAULT_DRAW_FBO {
            let rbo_id = self.fbos[&self.bound_draw_fbo].rbo;
            (
                &self.fbos[&self.bound_draw_fbo].fbo,
                self.fbos[&self.bound_draw_fbo].format,
        ...
        let cmd_buffer = self.command_pool[self.next_id].command_buffer_mut();
        unsafe {
            assert!(self.draw_target_usage == DrawTargetUsage::Draw
                || self.draw_target_usage == DrawTargetUsage::ClearOnly);
