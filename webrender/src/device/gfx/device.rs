...
impl<B: hal::Backend> Device<B> {
    ...
    pub fn read_pixels_into(
        ...
        unsafe {
            ...
            if let Some((barrier, pipeline_stages)) = image.transit_back() {
                cmd_buffer.pipeline_barrier(
                    pipeline_stages,
                    hal::memory::Dependencies::empty(),
                    &[barrier],
    ...
    pub fn begin_render_pass(&mut self, last_pass_before_submit: bool) {
        let dst_layout = if last_pass_before_submit {
    ...
    fn clear_target_image(&mut self, color: Option<[f32; 4]>, depth: Option<f32>) {
        ...
        unsafe {
            ...
            if let (Some(depth), Some(dimg)) = (depth, dimg) {
                ...
                if let Some((barrier, pipeline_stages)) = dimg.transit_back() {
                    self.command_buffer.pipeline_barrier(
                        pipeline_stages,
                        hal::memory::Dependencies::empty(),
                        &[barrier],
