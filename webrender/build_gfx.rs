...
fn create_vertex_buffer_descriptors(file_name: &str) -> Vec<VertexBufferDesc> {
    ...
    } else if file_name.starts_with("cs_gradient") {
        descriptors.push(
            VertexBufferDesc {
                binding: 1,
                stride: mem::size_of::<GradientInstance>() as _,
                rate: VertexInputRate::Instance(1),
            }
        );
    } else if file_name.starts_with("cs_svg") {
        descriptors.push(
            VertexBufferDesc {
                binding: 1,
                stride: mem::size_of::<SvgFilterInstance>() as _,
                rate: VertexInputRate::Instance(1),
            }
        );
    ...
    } else if file_name.starts_with("composite") {
        descriptors.push(
            VertexBufferDesc {
                binding: 1,
                stride: mem::size_of::<CompositeInstance>() as _,
                rate: VertexInputRate::Instance(1),
            }
        );
    } else if file_name.starts_with("pls") {
        descriptors.push(
            VertexBufferDesc {
                binding: 1,
                stride: mem::size_of::<PlsInstance>() as _,
                rate: VertexInputRate::Instance(1),
            }
        );
...
fn compile_glsl_to_spirv(file_name_vector: Vec<String>, out_dir: &str, shader_file_path: &Path) ->  HashMap<String, PipelineRequirements> {
    ...
    for (_index, file_name) in file_name_vector.iter().enumerate() {
        ...
        write!(
            shader_file,
            "    h.insert(\"{}\", gfx_hal::read_spirv(&std::fs::File::open(\"{}\").unwrap()).unwrap());\n",
            file_name,
            spirv_file_path,
        ).unwrap();
    }
